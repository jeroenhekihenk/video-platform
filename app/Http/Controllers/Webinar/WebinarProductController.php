...
class WebinarProductController extends Controller {
	...
	public function getHerhalingWebinar()
	{
		...
		if(Carbon::now() > $webinardatum && Carbon::now() < $voorwebinardatum) {
			if(Auth::user()){
				return view('webinars.iborawebinar-herhaling');
			} else {
				return redirect('VPNO/webinar')->with(['message'=>'Je moet je eerst aanmelden voor je de herhaling mag zien.']);
			}
		} else {
	...
	public function getVPNO()
	{
		$webinardatum = Carbon::create(2015, 10, 24, '12', '00', '00', 'Europe/Amsterdam');
		$voorwebinardatum = Carbon::create(2015,10,25, '23', '59', '59', 'Europe/Amsterdam');
		if(Carbon::now() > $webinardatum && Carbon::now() < $voorwebinardatum) {
			// Laat aanmeldpagina voor Ibora's Seminar zien

			return view('webinars.ibora');
		} else {
			return view('webinars.ibora-telaat');
	...
	public function postVPNO(IboraAanmeldRequest $request, User $user)
	{
		$webinardatum = Carbon::create(2015, 10, 24, '12', '00', '00', 'Europe/Amsterdam');
		$voorwebinardatum = Carbon::create(2015,10,25, '23', '59', '59', 'Europe/Amsterdam');
		if(Carbon::now() > $webinardatum && Carbon::now() < $voorwebinardatum) {
			...
			// Mail::send('includes.email.webinars.VPNO', $data, function($message) use($data)
			// {
			...
			Mail::send('includes.email.webinars.VPNO', $data, function($message) use($data)
			{
				...
				$message->to('info@digitusmarketing.nl', 'Digitus Marketing')
					->subject($data['voornaam']. ' ' .$data['achternaam']. ' heeft zich zojuist aangemeld voor het VPNO Webinar');
			});

			// return redirect('VPNO/bedankt')->with(['message'=>'Je hebt je succesvol aangemeld voor ons webinar! Bekijk je inbox voor verdere instructies.']);
		}
	}

	public function getVPNOBedankt()
	{
		$webinardatum = Carbon::create(2015, 10, 24, '12', '00', '00', 'Europe/Amsterdam');
		$voorwebinardatum = Carbon::create(2015,10,25, '23', '59', '59', 'Europe/Amsterdam');
		if(Carbon::now() > $webinardatum && Carbon::now() < $voorwebinardatum) {
			...
			return view('webinars.iborabedankt');
		}
	}
